...
def preprocess(particle_type, position_seq, target_position, metadata, noise_std):
    ...
    edge_displacement = torch.gather(
        recent_position, dim=0, index=edge_index[0].unsqueeze(-1).expand(-1, dim)
    ...
    graph = pyg.data.Data(
        ...
        y=acceleration,
        pos=torch.cat(
            (velocity_seq.reshape(velocity_seq.size(0), -1), distance_to_boundary), dim=-1
        ),
    )
    return graph
...
class OneStepDataset(pyg.data.Dataset):
    def __init__(self, data_path, split, window_length=7, noise_std=0.0):
        ...
        self.particle_type = np.memmap(
            os.path.join(data_path, f"{split}_particle_type.dat"), dtype=np.int64, mode="r"
        )
        self.position = np.memmap(
            os.path.join(data_path, f"{split}_position.dat"), dtype=np.float32, mode="r"
        )
        if os.path.exists(os.path.join(data_path, f"{split}_step_context.dat")):
            self.step_context = np.memmap(
                os.path.join(data_path, f"{split}_step_context.dat"), dtype=np.float32, mode="r"
            )
        else:
    ...
    def get(self, idx):
        ...
        if self.step_context:
            step_context = self.step_context[
                window["global"] : window["global"] + self.window_length * self.global_feature_dim
            ].copy()
        ...
        with torch.no_grad():
            graph = preprocess(
                particle_type, position_seq, target_position, self.metadata, self.noise_std
            )
        return graph
...
class RolloutDataset(pyg.data.Dataset):
    ...
    def get(self, idx):
        ...
        particle_type = self.particle_type[
            traj["particle_type"]["offset"] : traj["particle_type"]["offset"] + size
        ].copy()
        particle_type = torch.from_numpy(particle_type)
        position = self.position[
            traj["position"]["offset"] : traj["position"]["offset"] + time_step * size * self.dim
        ].copy()
        ...
        if self.step_context:
            step_context = self.step_context[
                traj["step_context"]["offset"] : traj["step_context"]["offset"]
                + time_step * self.global_feature_dim
            ].copy()
