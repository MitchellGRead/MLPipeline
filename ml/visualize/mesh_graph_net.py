...
def make_animation(gs, pred, evl, skip=2):
    """
    ...
    def animate(num):
        ...
        for ax in axes:
            ...
            if count <= 1:
                ...
                # error: (pred - gs)/gs
                mesh_plot = ax.tripcolor(
                    triang, velocity[:, 0], vmin=bb_min_evl, vmax=bb_max_evl, shading="flat"
                )  # x-velocity
                ax.triplot(triang, "ko-", ms=0.5, lw=0.3)
    ...
    return animation.FuncAnimation(fig, animate, frames=num_frames, interval=1000)


def visualize(loader, best_model, stats_list, delta_t=0.01, skip=1):
    ...
    return make_animation(gs_data_loader, viz_data_loader, eval_data_loader, skip)
