...
class MeshGraphNet(torch.nn.Module):
    def __init__(
        self, input_dim_node, input_dim_edge, hidden_dim, output_dim, num_layers, emb=False
    ):
        ...
        for _ in range(self.num_layers):
            edge_procssor_layer, node_processor_layer = self.build_processor_model(
    ...
    def forward(self, data, mean_vec_x, std_vec_x, mean_vec_edge, std_vec_edge):
        ...
        """
        x, edge_index, edge_attr, _ = data.x, data.edge_index, data.edge_attr, data.p

        x = mesh_preprocessor.normalize(x, mean_vec_x, std_vec_x)
        edge_attr = mesh_preprocessor.normalize(edge_attr, mean_vec_edge, std_vec_edge)
    ...
    def loss(self, pred, inputs, mean_vec_y, std_vec_y):
        ...
        loss_mask = torch.logical_or(
            (torch.argmax(inputs.x[:, 2:], dim=1) == torch.tensor(0)),
            (torch.argmax(inputs.x[:, 2:], dim=1) == torch.tensor(5)),
        )

        labels = mesh_preprocessor.normalize(inputs.y, mean_vec_y, std_vec_y)
...
class NodeProcessorLayer(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr, size=None):
        ...
        out = self.propagate(
            edge_index, edge_attr=edge_attr, size=size
        )  # out has the shape of [E, out_channels]
